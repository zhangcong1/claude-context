...
import { EmbeddingService, SemanticSearchResult } from './embeddingService';
...
export interface SearchOptions {
    ...
    keywordWeight: number;      // 关键词匹配权重 (0-1)
    semanticWeight: number;     // 语义匹配权重 (0-1)
    maxResults: number;         // 最大结果数
    includeFiles: boolean;      // 是否包含文件名匹配
...
export interface EnhancedSearchResult extends SemanticSearchResult {
    ...
    snippets: string[];         // 匹配的代码片段
    explanation: string;        // 匹配原因说明
}
...
export class SemanticSearchService {
    ...
    private semanticIndex = new Map<string, SemanticIndex>();
     ...
     * 为所有节点建立语义索引
     */
    async buildSemanticIndex(nodes: GraphNode[]): Promise<void> {
        if (this.isIndexing) {
     ...
     * 增量更新语义索引
     */
    async updateNodeIndex(node: GraphNode): Promise<void> {
    ...
    async semanticSearch(
        ...
        for (const node of nodes) {
            ...
            const relevanceScore = 
                keywordScore * searchOptions.keywordWeight +
     ...
     * 计算关键词匹配分数
     */
    private calculateKeywordScore(query: string, node: GraphNode, options: SearchOptions): number {
     ...
     * 计算模糊匹配分数
     */
    private calculateFuzzyScore(query: string, node: GraphNode): number {
        // 简单的编辑距离算法
    ...
    private determineMatchType(
        ...
        if (keywordScore > 0.8) return 'exact';
        if (semanticScore > options.semanticThreshold && keywordScore > 0.3) return 'hybrid';
        if (semanticScore > options.semanticThreshold) return 'semantic';
        if (fuzzyScore > 0.7) return 'fuzzy';
        return 'hybrid';
    }
     ...
     * 提取匹配的代码片段
     */
    private extractMatchingSnippets(query: string, node: GraphNode): string[] {
     ...
     * 检查上下文匹配
     */
    private hasContextMatch(query: string, node: GraphNode): boolean {
        const contextFields = [
            node.semantic,
     ...
     * 提取可搜索的文本
     */
    private extractSearchableText(node: GraphNode): string {
        const parts = [
            node.name,
            node.type,
            node.semantic || '',
     ...
     * 获取索引状态
     */
    getIndexStatus(): { size: number; isIndexing: boolean } {
        return {
