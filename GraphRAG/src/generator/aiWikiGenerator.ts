...
class AIWikiGenerator extends WikiGenerator {
    ...
    async analyzeCodeWithAI(codeSnippet: any, nodeInfo: any) {
    ...
    // 生成建议
    generateRecommendations(nodeInfo: any) {
    ...
    // 生成使用示例
    generateUsageExample(nodeInfo: any) {
        const type = nodeInfo.type;
        const name = nodeInfo.name;
...
`;
        ...
        if (classes.length > 0) {
            ...
            for (const cls of classes) {
                ...
                // 查找类的方法和继承关系
                const classEdges = this.knowledgeGraph.getAllEdges().filter((edge: any) => 
                    edge.source === cls.id || edge.target === cls.id
                );
                
                if (classEdges.length > 0) {
                    apiDocs += `**关系分析**:\n`;
                    const inheritEdges = classEdges.filter((e: any) => e.relation === 'inherits');
                    const callEdges = classEdges.filter((e: any) => e.relation === 'calls');
                    
                    if (inheritEdges.length > 0) {
                        inheritEdges.forEach((edge: any) => {
...
`;
        ...
        if (functions.length > 0) {
            ...
            for (const func of functions) {
                ...
                // 查找函数的调用关系
                const funcCalls = this.knowledgeGraph.getAllEdges().filter((edge: any) => 
                    edge.source === func.id && edge.relation === 'calls'
                );
                
                if (funcCalls.length > 0) {
                    apiDocs += `**调用的函数**:\n`;
                    funcCalls.slice(0, 5).forEach((edge: any) => { // 限制显示前5个
                        apiDocs += `- \`${edge.target}\`\n`;
                    });
...
`;
    ...
    const hasTypeScript = this.projectAnalysis.modules.some((m: any) => m.file.endsWith('.ts'));
