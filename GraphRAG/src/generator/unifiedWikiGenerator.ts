const { parseTsFile } = require('../parser');
...
class UnifiedWikiGenerator {
    knowledgeGraph: any;
    analysis: any;
    codeComments: Map<any, any>;
    fileContents: Map<any, any>;
    ...
    // 统一分析项目
    async analyzeProject(projectFiles: string[]) {
    ...
    extractComments(filePath: string, content: string) {
        const comments: any[] = [];
    ...
    analyzeTechnical() {
        const allNodes: any[] = this.knowledgeGraph.getAllNodes();
        const allEdges: any[] = this.knowledgeGraph.getAllEdges();
        ...
        // 依赖分析
        const importEdges: any[] = allEdges.filter((edge: any) => edge.relation === 'imports');
        this.analysis.technical.dependencies = importEdges.map((edge: any) => ({
    ...
    analyzeBusiness() {
        ...
        allNodes.forEach((node: any) => {
            const comments: any[] = this.codeComments.get(node.file) || [];
    ...
    analyzeWithAI() {
        const stats: any = this.knowledgeGraph.getStats();
        const allEdges: any[] = this.knowledgeGraph.getAllEdges();
        ...
        if (this.analysis.technical.modules.some((m: any) => m.file.endsWith('.ts'))) this.analysis.ai.techStack.push('TypeScript');
    ...
    // 辅助方法
    isBusinessFeature(node: any) {
        const keywords = ['login', 'register', 'user', 'order', 'payment', 'search'];
        return keywords.some((keyword: string) => node.name.toLowerCase().includes(keyword));
    }

    inferBusinessPurpose(name: string) {
    ...
    extractBusinessDescription(node: any, comments: any[]) {
        const relevantComment = comments.find((c: any) => Math.abs(c.line - (node.position?.line || 0)) <= 3);
        return relevantComment ? relevantComment.content : this.inferBusinessPurpose(node.name);
    }

    extractInterfaceProperties(node: any) {
    ...
    inferDataModelRole(name: string) {
        const mappings: {[key: string]: string} = {
    ...
    isApiFunction(node: any) {
        const content = this.fileContents.get(node.file) || '';
        return (node.type === 'Function' && content.includes('fetch')) ||
               ['get', 'post', 'put', 'delete'].some((method: string) => node.name.toLowerCase().includes(method));
    }

    extractHttpMethod(node: any) {
    ...
    inferComponentRole(name: string) {
        const mappings: {[key: string]: string} = {
    ...
    analyzeBusinessWorkflows(edges: any[]) {
        const workflows = new Map();
        edges.filter((edge: any) => edge.relation === 'calls').forEach((edge: any) => {
        ...
        workflows.forEach((steps: string[], name: string) => {
            if (steps.length >= 2) {
...
`;
        this.analysis.technical.modules.forEach((module: any) => {
            docs += `### ${path.basename(module.file)}\n`;
            docs += `- **节点数量**: ${module.nodeCount}\n`;
            docs += `- **主要类型**: ${module.nodes.map((n: any) => n.type).join(', ')}\n\n`;
        });

        docs += `## 🏗️ 架构分层

### 前端层 (${this.analysis.technical.architecture.frontend.length} 个组件)
${this.analysis.technical.architecture.frontend.map((node: any) => 
    `- **${node.name}**: ${node.type}`
).join('\n') || '暂无前端组件'}

### 服务层 (${this.analysis.technical.architecture.backend.length} 个服务)  
${this.analysis.technical.architecture.backend.map((node: any) => 
    `- **${node.name}**: ${node.type}`
).join('\n') || '暂无后端服务'}

### 配置层 (${this.analysis.technical.architecture.config.length} 个配置)
${this.analysis.technical.architecture.config.map((node: any) => 
    `- **${node.name}**: ${node.type}`
).join('\n') || '暂无配置文件'}

## 🔗 依赖关系

${this.analysis.technical.dependencies.slice(0, 10).map((dep: any) => 
...
`;
        this.analysis.business.features.forEach((feature: any) => {
            docs += `### ${feature.name}\n`;
...
`;
        this.analysis.business.dataModels.forEach((model: any) => {
            docs += `### ${model.name}\n`;
...
`;
        this.analysis.business.apis.forEach((api: any) => {
            docs += `### ${api.name}\n`;
...
`;
        this.analysis.business.components.forEach((component: any) => {
            docs += `### ${component.name}\n`;
...
### 🛠️ 技术栈识别
${this.analysis.ai.techStack.map((tech: string) => `- ✅ **${tech}**: 已识别使用`).join('\n') || '- 暂未识别到特定技术栈'}
...
${this.analysis.ai.recommendations.map((rec: string, index: number) => 
    `${index + 1}. ${rec}`
).join('\n') || '暂无特定优化建议'}
