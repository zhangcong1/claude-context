...
class WikiGenerator {
    ...
    analyzeArchitecture() {
        ...
        this.projectAnalysis.dependencies = Array.from(dependencies.entries()).map(([file, deps]: [string, any[]]) => ({
            file,
    ...
    generateOverview() {
        ...
        const relationDescriptions: {[key: string]: string} = {
            'imports': '模块导入关系',
            'calls': '函数调用关系', 
            'inherits': '类继承关系',
            'references': '引用关系'
...
| 类型 | 数量 | 占比 |
|------|------|------|
${Object.entries(stats.nodeTypes).map(([type, count]: [string, any]) => 
...
| 关系类型 | 数量 | 说明 |
|----------|------|------|
${Object.entries(stats.relationTypes).map(([relation, count]: [string, any]) => {
    return `| ${relation} | ${count} | ${relationDescriptions[relation] || '其他关系'} |`;
}).join('\n')}
`;
    }
...
`;
        ...
        this.knowledgeGraph.getAllNodes().forEach((node: any) => {
            const file = node.file;
        ...
        Array.from(modulesByFile.entries()).forEach(([file, nodes]: [string, any[]]) => {
            const fileName = path.basename(file);
            ...
            // 文件的依赖关系
            const fileDeps = this.projectAnalysis.dependencies.find((d: any) => d.file === file);
            if (fileDeps && fileDeps.dependencies.length > 0) {
                moduleDocs += `### 依赖关系\n\n`;
                fileDeps.dependencies.forEach((dep: string) => {
                    moduleDocs += `- \`${dep}\`\n`;
                });
                moduleDocs += '\n';
            }
...
`;
        ...
        if (classes.length > 0) {
            classes.forEach((cls: any) => {
                ...
                // 查找类的方法和继承关系
                const classEdges: any[] = this.knowledgeGraph.getAllEdges().filter((edge: any) => 
                    edge.source === cls.id || edge.target === cls.id
                );
                
                if (classEdges.length > 0) {
                    apiDocs += `**关系**:\n`;
                    classEdges.forEach((edge: any) => {
        ...
        if (functions.length > 0) {
            functions.forEach((func: any) => {
                ...
                // 查找函数的调用关系
                const funcCalls: any[] = this.knowledgeGraph.getAllEdges().filter((edge: any) => 
                    edge.source === func.id && edge.relation === 'calls'
                );
                
                if (funcCalls.length > 0) {
                    apiDocs += `**调用的函数**:\n`;
                    funcCalls.forEach((edge: any) => {
                        apiDocs += `- \`${edge.target}\`\n`;
                    });
                    apiDocs += '\n';
                }
...
graph TB
    subgraph "前端层"
        ${this.projectAnalysis.architecture.frontend.map((node: any) => 
            `${this.sanitizeId(node.id)}["${node.name}"]`
        ).join('\n        ')}
    end
    
    subgraph "服务层"
        ${this.projectAnalysis.architecture.backend.map((node: any) => 
            `${this.sanitizeId(node.id)}["${node.name}"]`
        ).join('\n        ')}
    end
    
    subgraph "配置层"
        ${this.projectAnalysis.architecture.config.map((node: any) => 
            `${this.sanitizeId(node.id)}["${node.name}"]`
        ).join('\n        ')}
    end
\`\`\`
...
`;
    ...
    generateDependencyMermaid() {
        ...
        return importEdges.map((edge: any) => {
            const sourceNode = this.knowledgeGraph.findNode(edge.source);
...
- **TypeScript模块**: ${this.projectAnalysis.modules.filter((m: any) => m.file.endsWith('.ts')).length} 个
...
### 📋 重要文件
${this.projectAnalysis.modules.slice(0, 5).map((module: any) => 
