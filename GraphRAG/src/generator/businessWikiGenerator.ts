const { parseTsFile, parseVueFile } = require('../parser');
...
class BusinessWikiGenerator {
    ...
    // 分析项目的业务逻辑
    async analyzeBusinessLogic(projectFiles: string[]) {
    ...
    getLineNumber(content: string, comment: string) {
    ...
    // 推断业务用途
    inferBusinessPurpose(node: any, content: string) {
    ...
    extractBusinessDescription(node: any, comments: any[]) {
        const relevantComment = comments.find((c: any) => 
    ...
    // 提取接口属性
    extractInterfaceProperties(node: any, content: string) {
    ...
    extractPropertyDescription(line: string) {
        const commentMatch = line.match(/\/\/\s*(.+)/);
        return commentMatch ? commentMatch[1].trim() : '';
    }

    // 判断是否为API函数
    isApiFunction(node: any, content: string) {
    ...
    extractHttpMethod(node: any, content: string) {
        const name = node.name.toLowerCase();
    ...
    extractApiDescription(node: any, comments: any[]) {
        const relevantComment = comments.find((c: any) => 
    ...
    extractComponentFeatures(filePath: string) {
    ...
    extractServiceMethods(node: any, allNodes: any[]) {
        return allNodes
            .filter((n: any) => n.file === node.file && n.type === 'Function')
            .slice(0, 5) // 限制方法数量
            .map((n: any) => ({
                name: n.name,
    ...
    generateWorkflowDescription(name: string) {
        const workflowMappings: {[key: string]: string} = {
            'login': '用户登录业务流程',
            'register': '用户注册业务流程',
            'order': '订单处理业务流程',
            'payment': '支付处理业务流程',
...
`;
        ...
        if (this.businessAnalysis.features.length > 0) {
            this.businessAnalysis.features.forEach((feature: any) => {
                docs += `### ${feature.name}\n\n`;
...
`;
        ...
        if (this.businessAnalysis.dataModels.length > 0) {
            this.businessAnalysis.dataModels.forEach((model: any) => {
                docs += `### ${model.name}\n\n`;
