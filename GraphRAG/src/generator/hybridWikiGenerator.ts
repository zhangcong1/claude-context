...
import { IntelligentWikiGenerator } from './intelligentWikiGenerator';
...
interface HybridModeConfig {
    ...
    structuralWeight: number;    // 结构分析权重 (0-1)
    traditionalWeight: number;   // 传统解析权重 (0-1)
    
    // 内容生成策略
    enableSemanticDiscovery: boolean;     // 启用语义功能发现
    enableStructuralAnalysis: boolean;    // 启用结构化分析
    enableTraditionalParsing: boolean;    // 启用传统解析
    
    // 文档生成选项
    generateArchitectureDocs: boolean;    // 生成架构文档
    generateFeatureDocs: boolean;         // 生成功能文档
    generateApiDocs: boolean;             // 生成API文档
    generateBusinessDocs: boolean;        // 生成业务文档
    
    // 质量控制
    minFeatureNodes: number;              // 功能最小节点数
    maxFeatureDepth: number;              // 功能分析最大深度
    enableContentValidation: boolean;     // 启用内容验证
}
...
interface HybridAnalysisResult {
    semanticFeatures: any[];              // 语义发现的功能
    structuralFeatures: any[];            // 结构分析的功能
    traditionalFeatures: any[];           // 传统解析的功能
    mergedFeatures: any[];                // 合并后的功能
 ...
 */
export class HybridWikiGenerator {
    private knowledgeGraph: KnowledgeGraph;
    private semanticSearch: SemanticSearchService;
    private intelligentGenerator: IntelligentWikiGenerator;
    private config: HybridModeConfig;

    constructor(knowledgeGraph: KnowledgeGraph, config?: Partial<HybridModeConfig>) {
        this.knowledgeGraph = knowledgeGraph;
        this.semanticSearch = new SemanticSearchService();
        this.intelligentGenerator = new IntelligentWikiGenerator(knowledgeGraph);
     ...
     * 生成混合模式Wiki
     */
    async generateHybridWiki(outputDir: string = './hybrid-wiki'): Promise<string> {
    ...
    private async buildSemanticIndex(): Promise<void> {
        if (!this.config.enableSemanticDiscovery) return;
     ...
     * 执行混合分析
     */
    private async performHybridAnalysis(): Promise<HybridAnalysisResult> {
        console.log('🔍 执行混合分析...');
        ...
        // 4. 智能合并功能
        result.mergedFeatures = await this.mergeFeatures(
            result.semanticFeatures,
            result.structuralFeatures,
            result.traditionalFeatures
        );
     ...
     * 语义功能发现
     */
    private async performSemanticDiscovery(): Promise<any[]> {
        const features: any[] = [];
     ...
     * 结构化分析
     */
    private async performStructuralAnalysis(): Promise<any[]> {
        const features: any[] = [];
        const allNodes = this.knowledgeGraph.getAllNodes();
     ...
     * 传统解析分析
     */
    private async performTraditionalAnalysis(): Promise<any[]> {
        const features: any[] = [];
        const allNodes = this.knowledgeGraph.getAllNodes();
    ...
    private async mergeFeatures(
        ...
        for (const feature of semanticFeatures) {
            const nodeIds = feature.relatedNodes?.map((n: GraphNode) => n.id) || [];
            ...
            // 合并重叠的功能
            const enhancedFeature = await this.enhanceFeatureWithOverlaps(
                feature,
                overlappingStructural,
                overlappingTraditional
            );

            mergedFeatures.push(enhancedFeature);
            nodeIds.forEach((id: string) => processedNodes.add(id));
        }

        // 2. 处理剩余的结构化功能
        for (const feature of structuralFeatures) {
            const nodeIds = feature.relatedNodes?.map((n: GraphNode) => n.id) || [];
            const hasUnprocessedNodes = nodeIds.some((id: string) => !processedNodes.has(id));
     ...
     * 生成分层文档
     */
    private async generateLayeredDocuments(analysisResult: HybridAnalysisResult): Promise<Record<string, string>> {
     ...
     * 生成混合模式概览
     */
    private async generateHybridOverview(analysisResult: HybridAnalysisResult): Promise<string> {
...
`;
     ...
     * 工具方法：获取功能来源标识
     */
    private getFeatureSourceBadge(source: string): string {
     ...
     * 其他必要的辅助方法
     */
    private async buildFeatureFromSearchResults(query: string, searchResults: EnhancedSearchResult[]): Promise<any> {
        // 实现从搜索结果构建功能的逻辑
        return {
            name: query,
    ...
    private calculateFeatureConfidence(searchResults: EnhancedSearchResult[]): number {
    ...
    private analyzeByModules(nodes: GraphNode[]): any[] {
        // 基于模块的分析逻辑
        return [];
    }

    private analyzeByCallRelations(nodes: GraphNode[], edges: GraphEdge[]): any[] {
        // 基于调用关系的分析逻辑
        return [];
    }

    private analyzeByInheritance(nodes: GraphNode[], edges: GraphEdge[]): any[] {
        // 基于继承关系的分析逻辑
        return [];
    }

    private analyzeVueComponents(nodes: GraphNode[]): any[] {
        // Vue组件分析逻辑
    ...
    private groupNodesByFile(nodes: GraphNode[]): Record<string, GraphNode[]> {
        return nodes.reduce((groups, node) => {
            if (!groups[node.file]) {
                groups[node.file] = [];
            }
    ...
    private analyzeNodeArchitecture(nodes: GraphNode[]): any {
        return {
    ...
    private findOverlappingFeatures(targetFeature: any, features: any[]): any[] {
        // 查找重叠功能的逻辑
        return [];
    }

    private async enhanceFeatureWithOverlaps(baseFeature: any, ...overlappingGroups: any[][]): Promise<any> {
        // 增强功能的逻辑
        return { ...baseFeature, source: 'hybrid' };
    }

    private calculateCoverage(features: any[], allNodes: GraphNode[]): any {
        const coveredNodeIds = new Set();
        features.forEach(feature => {
            feature.relatedNodes?.forEach((node: GraphNode) => {
    ...
    private calculateConfidence(result: HybridAnalysisResult): number {
    ...
    // 其他生成方法的占位符
    private async generateHybridArchitectureDocs(analysisResult: HybridAnalysisResult): Promise<string> {
        return "# 🏗️ 混合模式架构文档\n\n待实现...";
    }

    private async generateHybridFeaturesDocs(analysisResult: HybridAnalysisResult): Promise<string> {
        return "# 🚀 混合模式功能文档\n\n待实现...";
    }

    private async generateEnhancedFeatureDoc(feature: any): Promise<string> {
        return `# 📋 ${feature.name}\n\n待实现...`;
    }

    private async generateHybridApiDocs(): Promise<string> {
        return "# 🔧 混合模式API文档\n\n待实现...";
    }

    private async generateBusinessLogicDocs(analysisResult: HybridAnalysisResult): Promise<string> {
        return "# 💼 业务逻辑文档\n\n待实现...";
    }

    private async generateDetailedAnalysisReport(analysisResult: HybridAnalysisResult): Promise<string> {
        return "# 📊 详细分析报告\n\n待实现...";
    }

    private async validateAndOptimizeContent(documents: Record<string, string>): Promise<void> {
        // 内容验证和优化逻辑
    ...
    private async generateAnalysisReport(outputDir: string, analysisResult: HybridAnalysisResult): Promise<void> {
