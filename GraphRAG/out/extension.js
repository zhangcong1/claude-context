...
function activate(context) {
    ...
    // 初始化知识图谱
    knowledgeGraph = new graph_1.KnowledgeGraph();
    graphViewer = new graphViewer_1.GraphViewer(knowledgeGraph);
    configManager = configManager_1.ConfigManager.getInstance();
    searchCommand = new searchCommand_1.SearchCommand(knowledgeGraph);
    indexCommand = new indexCommand_1.IndexCommand(knowledgeGraph);
    // 注册"构建知识图谱"命令
    ...
    const showGraphCmd = vscode.commands.registerCommand('vscode-graphrag.showGraph', async () => {
        if (knowledgeGraph.getStats().nodeCount === 0) {
            const result = await vscode.window.showWarningMessage('知识图谱为空，是否先构建图谱？', '构建图谱', '查看空图谱');
            if (result === '构建图谱') {
                await buildKnowledgeGraph();
            }
        }
    ...
    const clearGraphCmd = vscode.commands.registerCommand('vscode-graphrag.clearGraph', async () => {
        const result = await vscode.window.showWarningMessage('确定要清空知识图谱吗？此操作不可撤销。', '确定', '取消');
        if (result === '确定') {
    ...
    const generateWikiCmd = vscode.commands.registerCommand('vscode-graphrag.generateWiki', async () => {
        await vscode.window.withProgress({
            ...
            try {
                ...
                vscode.window.showInformationMessage(`Wiki文档生成成功！位置: ${wikiDir}`, '打开文件夹').then(selection => {
                    if (selection === '打开文件夹') {
    ...
    const generateAIWikiCmd = vscode.commands.registerCommand('vscode-graphrag.generateAIWiki', async () => {
        await vscode.window.withProgress({
            ...
            try {
                ...
                vscode.window.showInformationMessage(`AI增强Wiki文档生成成功！位置: ${wikiDir}`, '打开文件夹', '查看AI分析报告').then(selection => {
                    if (selection === '打开文件夹') {
                        vscode.env.openExternal(vscode.Uri.file(wikiDir));
                    }
    ...
    const generateUnifiedWikiCmd = vscode.commands.registerCommand('vscode-graphrag.generateUnifiedWiki', async () => {
        await vscode.window.withProgress({
            ...
            try {
                ...
                vscode.window.showInformationMessage(`统一知识图谱Wiki生成成功！位置: ${outputPath}`, '打开文件', '查看概览').then(selection => {
                    if (selection === '打开文件') {
    ...
    const generateBusinessWikiCmd = vscode.commands.registerCommand('vscode-graphrag.generateBusinessWiki', async () => {
        await vscode.window.withProgress({
            ...
            try {
                ...
                vscode.window.showInformationMessage(`业务Wiki文档生成成功！位置: ${wikiDir}`, '打开文件夹', '查看业务功能').then(selection => {
                    if (selection === '打开文件夹') {
                        vscode.env.openExternal(vscode.Uri.file(wikiDir));
                    }
...
async function buildKnowledgeGraph() {
    ...
    await vscode.window.withProgress({
        ...
        vscode.window.showInformationMessage(`知识图谱构建完成！节点: ${stats.nodeCount}, 边: ${stats.edgeCount}`);
    });
}
...
function getAllCodeFiles(dir, rootPath) {
...
function setupFileWatcher(context) {
    ...
    // 创建文件监听器
    fileWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(workspaceFolders[0], '**/*.{ts,js,vue,json,md,yaml,yml,css,scss,less}'));
    // 监听文件保存事件
