...
class SearchCommand {
    ...
    constructor(knowledgeGraph) {
        ...
        this.semanticSearchService = new semanticSearchService_1.SemanticSearchService();
    }
    async execute() {
        // 首先检查是否有可用的节点
     ...
     * 执行语义搜索
     */
    async performSemanticSearch(query, progress, token) {
        ...
        const results = await this.semanticSearchService.semanticSearch(query, this.knowledgeGraph.getAllNodes(), searchOptions);
     ...
     * 执行混合搜索
     */
    async performHybridSearch(query, progress, token) {
     ...
     * 执行传统搜索
     */
    async performTraditionalSearch(query, progress) {
     ...
     * 计算传统搜索的相关性
     */
    calculateTraditionalRelevance(query, node) {
     ...
     * 提取简单代码片段
     */
    extractSimpleSnippets(query, node) {
        const snippets = [];
        if (node.snippet) {
     ...
     * 显示搜索结果
     */
    async showSearchResults(query, results, searchMode) {
        if (results.length === 0) {
            vscode.window.showInformationMessage(`未找到与 "${query}" 相关的节点。尝试使用不同的关键词或搜索模式。`);
            return;
        }
        // 创建快速选择项目
    ...
    async navigateToNode(node) {
        try {
            ...
            if (node.position) {
                const position = new vscode.Position(node.position.line, node.position.column);
