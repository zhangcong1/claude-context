...
exports.generateHybridWikiCommand = generateHybridWikiCommand;
exports.customizeHybridModeCommand = customizeHybridModeCommand;
const vscode = __importStar(require("vscode"));
const graph_1 = require("../graph");
const hybridWikiGenerator_1 = require("../generator/hybridWikiGenerator");
 ...
 */
async function generateHybridWikiCommand() {
    try {
        // 显示配置选项
        ...
        await vscode.window.withProgress({
            ...
            // 4. 创建混合模式生成器
            const hybridGenerator = new hybridWikiGenerator_1.HybridWikiGenerator(knowledgeGraph, config);
            // 选择输出目录
            const outputOptions = {
                canSelectMany: false,
            ...
            const result = await vscode.window.showInformationMessage(`🎉 混合模式Wiki生成成功！\n\n` +
                `📊 分析了 ${stats.nodeCount} 个代码实体\n` +
                `🔀 使用混合分析策略\n` +
                `📁 输出到: ${wikiPath}\n\n` +
                `是否现在打开Wiki目录？`, '打开目录', '查看概览', '查看分析报告', '关闭');
            if (result === '打开目录') {
            ...
            // 7. 显示混合模式说明
            vscode.window.showInformationMessage(`🔀 混合模式特色：\n` +
                `• 🤖 语义搜索：AI智能功能发现\n` +
                `• 🏗️ 结构分析：代码架构深度分析\n` +
                `• 📋 传统解析：完整覆盖所有代码\n` +
                `• 🎯 智能合并：多策略结果融合`, '了解');
 ...
 * 自定义混合模式配置命令
 */
async function customizeHybridModeCommand() {
    try {
        ...
        vscode.window.showInformationMessage(`✅ 混合模式配置已保存！\n\n` +
            `语义权重: ${(config.semanticWeight * 100).toFixed(0)}%\n` +
            `结构权重: ${(config.structuralWeight * 100).toFixed(0)}%\n` +
            `传统权重: ${(config.traditionalWeight * 100).toFixed(0)}%`, '开始生成Wiki').then(selection => {
 ...
 * 显示混合模式配置选项
 */
async function showHybridModeConfig() {
    const modeOptions = [
        {
            label: '🎯 平衡模式',
 ...
 * 显示高级混合模式配置
 */
async function showAdvancedHybridConfig() {
    // 语义搜索权重
...
//# sourceMappingURL=hybridWikiCommand.js.map