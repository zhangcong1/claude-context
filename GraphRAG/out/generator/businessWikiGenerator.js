...
class BusinessWikiGenerator {
    ...
    extractHttpMethod(node, content) {
        const name = node.name.toLowerCase();
        if (name.includes('get') || content.includes("'GET'"))
            return 'GET';
        if (name.includes('post') || content.includes("'POST'"))
            return 'POST';
        if (name.includes('put') || content.includes("'PUT'"))
            return 'PUT';
        if (name.includes('delete') || content.includes("'DELETE'"))
            return 'DELETE';
        return 'GET';
    }
    extractApiDescription(node, comments) {
        const relevantComment = comments.find((c) => Math.abs(c.line - (node.position?.line || 0)) <= 2);
...
//# sourceMappingURL=businessWikiGenerator.js.map