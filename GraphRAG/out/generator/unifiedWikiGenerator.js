...
class UnifiedWikiGenerator {
    ...
    analyzeTechnical() {
        ...
        // 架构分析
        allNodes.forEach((node) => {
            if (node.file.endsWith('.vue'))
                this.analysis.technical.architecture.frontend.push(node);
            else if (node.file.includes('service'))
                this.analysis.technical.architecture.backend.push(node);
            else if (node.file.endsWith('.json'))
                this.analysis.technical.architecture.config.push(node);
        });
        // 依赖分析
    ...
    analyzeWithAI() {
        ...
        if (this.analysis.technical.architecture.frontend.length > 0)
            this.analysis.ai.techStack.push('Vue.js');
        if (this.analysis.technical.modules.some((m) => m.file.endsWith('.ts')))
            this.analysis.ai.techStack.push('TypeScript');
        // 架构模式识别
        ...
        if (this.analysis.ai.complexity.level === '高')
            this.analysis.ai.recommendations.push('代码复杂度较高，建议拆分大型模块');
        if (this.analysis.ai.techStack.includes('Vue.js'))
            this.analysis.ai.recommendations.push('使用Composition API提高代码复用性');
...
${this.analysis.technical.architecture.frontend.map((node) => `- **${node.name}**: ${node.type}`).join('\n') || '暂无前端组件'}

### 服务层 (${this.analysis.technical.architecture.backend.length} 个服务)  
${this.analysis.technical.architecture.backend.map((node) => `- **${node.name}**: ${node.type}`).join('\n') || '暂无后端服务'}

### 配置层 (${this.analysis.technical.architecture.config.length} 个配置)
${this.analysis.technical.architecture.config.map((node) => `- **${node.name}**: ${node.type}`).join('\n') || '暂无配置文件'}

## 🔗 依赖关系

${this.analysis.technical.dependencies.slice(0, 10).map((dep) => `- ${dep.source} → ${dep.target}`).join('\n') || '暂无依赖关系'}
`;
        return docs;
    }
    // 生成业务功能文档
...
${this.analysis.ai.recommendations.map((rec, index) => `${index + 1}. ${rec}`).join('\n') || '暂无特定优化建议'}
...
//# sourceMappingURL=unifiedWikiGenerator.js.map