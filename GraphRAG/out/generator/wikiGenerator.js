...
class WikiGenerator {
    ...
    projectAnalysis;
    constructor() {
        this.knowledgeGraph = new graph_1.KnowledgeGraph();
    ...
    async analyzeProject(projectFiles) {
        ...
        for (const file of projectFiles) {
            try {
                const result = (0, parser_1.parseTsFile)(file);
...
| 类型 | 数量 | 占比 |
|------|------|------|
${Object.entries(stats.nodeTypes).map(([type, count]) => `| ${type} | ${count} | ${(count / stats.nodeCount * 100).toFixed(1)}% |`).join('\n')}
...
graph TB
    subgraph "前端层"
        ${this.projectAnalysis.architecture.frontend.map((node) => `${this.sanitizeId(node.id)}["${node.name}"]`).join('\n        ')}
    end
    
    subgraph "服务层"
        ${this.projectAnalysis.architecture.backend.map((node) => `${this.sanitizeId(node.id)}["${node.name}"]`).join('\n        ')}
    end
    
    subgraph "配置层"
        ${this.projectAnalysis.architecture.config.map((node) => `${this.sanitizeId(node.id)}["${node.name}"]`).join('\n        ')}
    end
\`\`\`
...
${this.projectAnalysis.architecture.frontend.length > 0 ?
            this.projectAnalysis.architecture.frontend.map((node) => `- **${node.name}**: ${node.type} (${path.basename(node.file)})`).join('\n') :
            '- 暂无前端组件'}

### 服务逻辑层
${this.projectAnalysis.architecture.backend.length > 0 ?
            this.projectAnalysis.architecture.backend.map((node) => `- **${node.name}**: ${node.type} (${path.basename(node.file)})`).join('\n') :
            '- 暂无服务组件'}

### 配置管理层
${this.projectAnalysis.architecture.config.length > 0 ?
            this.projectAnalysis.architecture.config.map((node) => `- **${node.name}**: ${node.type} (${path.basename(node.file)})`).join('\n') :
            '- 暂无配置文件'}
`;
        return archDocs;
    }
...
### 📋 重要文件
${this.projectAnalysis.modules.slice(0, 5).map((module) => `- [${module.name}](./modules.md#${module.name.toLowerCase()}) - ${module.type}`).join('\n')}
