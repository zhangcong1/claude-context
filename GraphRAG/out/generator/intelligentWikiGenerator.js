...
exports.IntelligentWikiGenerator = void 0;
 ...
 * 4. AI增强的文档生成
 */
class IntelligentWikiGenerator {
    knowledgeGraph;
    semanticSearch;
     ...
     * 生成完整的项目Wiki
     */
    async generateCompleteWiki(outputDir = './intelligent-wiki') {
     ...
     */
    async analyzeFeature(featureQuery) {
        // 🔥 关键步骤1: 语义搜索相关节点
        const searchResults = await this.semanticSearch.semanticSearch(featureQuery, this.knowledgeGraph.getAllNodes(), {
     ...
     */
    async collectCompleteCode(nodes) {
     ...
     * 分析代码片段，提取详细信息
     */
    analyzeCodeSnippet(snippet, fullCode) {
        const analysis = {
     ...
     * 分析功能架构
     */
    analyzeFeatureArchitecture(nodes) {
        const architecture = {
            entry: [],
            core: [],
            helpers: []
        ...
        for (const node of nodes) {
            ...
            // 入口节点：主要的对外接口
            if (node.type === 'Function' && (name.includes('main') ||
                name.includes('init') ||
            ...
            // 辅助节点：工具函数
            else if (node.type === 'Function' && (name.includes('util') ||
     ...
     * 分析执行流程
     */
    async analyzeExecutionFlows(nodes) {
        const flows = [];
        // 找到入口节点
        const entryNodes = nodes.filter(node => node.type === 'Function' || node.type === 'VueMethod' && (node.name.includes('handle') ||
     ...
     * 追踪详细执行流程，包括业务逻辑分析
     */
    async traceDetailedExecutionFlow(entryNode, relatedNodes) {
        const visited = new Set();
        const steps = [];
        const traceNode = (node, depth = 0) => {
            if (visited.has(node.id) || depth > 5)
                return;
            visited.add(node.id);
            // 分析节点的业务逻辑
            ...
            steps.push({
                ...
                businessLogic,
                complexity: node.metadata?.codeComplexity || 'simple',
                externalCalls: node.metadata?.externalCalls || []
            });
            // 查找调用的下一个节点
            const callEdges = this.knowledgeGraph.getAllEdges().filter(edge => edge.source === node.id && edge.relation === 'calls');
     ...
     * 生成详细的步骤描述
     */
    generateDetailedStepDescription(node, businessLogic) {
     ...
     */
    async generateFeatureDetailDoc(feature) {
        return `# 📋 ${feature.name}
...
${feature.architecture.entry.map(node => `- **${node.name}** (${node.type}): ${path.basename(node.file)}:${(node.position?.line || 0) + 1}`).join('\n')}

### 核心逻辑 (${feature.architecture.core.length}个)
${feature.architecture.core.map(node => `- **${node.name}** (${node.type}): ${path.basename(node.file)}:${(node.position?.line || 0) + 1}`).join('\n')}

### 辅助函数 (${feature.architecture.helpers.length}个)
${feature.architecture.helpers.map(node => `- **${node.name}** (${node.type}): ${path.basename(node.file)}:${(node.position?.line || 0) + 1}`).join('\n')}
...
## 代码实体分布
${Object.entries(stats.nodeTypes).map(([type, count]) => `- **${type}**: ${count} 个`).join('\n')}

## 关系类型统计
${Object.entries(stats.relationTypes).map(([relation, count]) => `- **${relation}**: ${count} 个关系`).join('\n')}
...
`;
     ...
     * 生成功能概览
     */
    async generateFeaturesOverview(features) {
        return `# 🚀 功能概览
...
`;
     ...
     * 工具方法：生成功能名称
     */
    generateFeatureName(query, nodes) {
     ...
     * 工具方法：生成功能描述
     */
    async generateFeatureDescription(query, nodes) {
...
exports.IntelligentWikiGenerator = IntelligentWikiGenerator;
//# sourceMappingURL=intelligentWikiGenerator.js.map